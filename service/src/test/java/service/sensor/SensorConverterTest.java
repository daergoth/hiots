...
public class SensorConverterTest {
	...
	public static void setUpBeforeClass() {
		...
		tempDummyVO = new DummyTemperatureSensorVO(7l, "DummyTemperatureSensor", 20, 25, 1000);

		lightDummyVO = new DummyLightSensorVO(8l, "DummyLightSensor", 10, 20, 1000);
	...
	public void testToDummyVO() {
		...
		try {
			...
			if (!(newDummyVo instanceof DummySensorVO)) {
				...
				Assert.assertEquals("Not matching lower limit for Dummy!", tempDummyVO.getMinData(),
						dummyVo.getMinData(), 0.0);

				Assert.assertEquals("Not matching upper limit for Dummy!", tempDummyVO.getMaxData(),
						dummyVo.getMaxData(), 0.0);

				Assert.assertEquals("Not matchong update interval for Dummy!", tempDummyVO.getInterval(),
						dummyVo.getInterval());
			...
			if (!(newDummyVo instanceof DummySensorVO)) {
				...
				Assert.assertEquals("Not matching lower limit for Dummy!", lightDummyVO.getMinData(),
						dummyVo.getMinData(), 0.0);

				Assert.assertEquals("Not matching upper limit for Dummy!", lightDummyVO.getMaxData(),
						dummyVo.getMaxData(), 0.0);

				Assert.assertEquals("Not matchong update interval for Dummy!", lightDummyVO.getInterval(),
						dummyVo.getInterval());
	...
	public void testToDummyDTO() {
		...
		if (!(newDto instanceof DummySensorDTO)) {
			...
			Assert.assertEquals("Not matching lower limit for Dummy!", tempDummyDTO.getMin(), newDummyDto.getMin(),
					0.0);

			Assert.assertEquals("Not matching upper limit for Dummy!", tempDummyDTO.getMax(), newDummyDto.getMax(),
					0.0);

			Assert.assertEquals("Not matchong update interval for Dummy!", tempDummyDTO.getInterval(),
					newDummyDto.getInterval());
